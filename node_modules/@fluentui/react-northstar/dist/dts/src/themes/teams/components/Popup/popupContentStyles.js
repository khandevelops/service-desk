"use strict";
var __assign = (this && this.__assign) || function () {
    __assign = Object.assign || function(t) {
        for (var s, i = 1, n = arguments.length; i < n; i++) {
            s = arguments[i];
            for (var p in s) if (Object.prototype.hasOwnProperty.call(s, p))
                t[p] = s[p];
        }
        return t;
    };
    return __assign.apply(this, arguments);
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.popupContentStyles = void 0;
var getPointerStyles_1 = require("../../getPointerStyles");
exports.popupContentStyles = {
    root: function (_a) {
        var p = _a.props, v = _a.variables;
        return (__assign(__assign({ display: 'block', zIndex: v.zIndex }, (p.pointing && __assign({ pointerEvents: 'none' }, getPointerStyles_1.getContainerStyles({
            placement: p.basePlacement,
            padding: v.pointerMargin,
        })))), (p.autoSize && {
            // when autoSize is true, root will have inline style maxWidth and maxHeight set by popper
            display: 'flex',
        })));
    },
    pointer: function (_a) {
        var p = _a.props, v = _a.variables, rtl = _a.rtl;
        return getPointerStyles_1.getPointerStyles({
            backgroundColor: v.backgroundColor,
            borderColor: v.borderColor,
            borderSize: v.borderSize,
            gap: v.pointerGap,
            padding: v.pointerMargin,
            height: v.pointerHeight,
            width: v.pointerWidth,
            placement: p.basePlacement,
            rtl: rtl,
        });
    },
    content: function (_a) {
        var p = _a.props, v = _a.variables;
        return (__assign(__assign({ display: 'block', background: v.backgroundColor, color: v.color, boxShadow: v.boxShadow, border: v.borderSize + " solid " + v.borderColor, borderRadius: v.borderRadius, padding: v.padding }, (p.pointing && {
            pointerEvents: 'all',
        })), (p.autoSize && {
            overflow: 'auto',
        })));
    },
};
