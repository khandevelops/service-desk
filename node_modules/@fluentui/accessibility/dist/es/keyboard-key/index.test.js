...
describe('keyboardKey', function () {
  it('has a key/value for every value/key in codes', function () {
    Object.keys(codes).forEach(function (code) {
      var name = codes[Number(code)];
      if (Array.isArray(name)) {
        expect(String(keyboardKey[name[0]])).toEqual(code);
        expect(String(keyboardKey[name[1]])).toEqual(code);
      } else {
        expect(String(keyboardKey[name])).toEqual(code);
      }
    });
  });
  ...
  describe('getKey', function () {
    ...
    it('handles all codes', function () {
      Object.keys(codes).forEach(function (code) {
        var name = codes[code];
        var keyName = getKey(Number(code));
        if (Array.isArray(name)) {
