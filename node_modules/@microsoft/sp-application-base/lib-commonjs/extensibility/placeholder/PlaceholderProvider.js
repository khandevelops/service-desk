...
var PlaceholderManager_1 = tslib_1.__importDefault(require("./PlaceholderManager"));
...
var PlaceholderProvider = /** @class */ (function () {
    ...
    function PlaceholderProvider(serviceScope, applicationCustomizerContextParameters, componentTag) {
        ...
        var qosMonitor = new sp_diagnostics_1._QosMonitor('PlaceholderProvider');
         ...
         */
        this._placeholderManager = serviceScope.consume(PlaceholderManager_1.default.serviceKey);
        if (preAllocatedApplicationCustomizerTopHeight > 0) {
            this._preAllocatedTopPlaceholderContent = this._preAllocateContent(PlaceholderName_1.default.Top, preAllocatedApplicationCustomizerTopHeight, qosMonitor);
        }
        if (preAllocatedApplicationCustomizerBottomHeight > 0) {
            this._preAllocatedBottomPlaceholderContent = this._preAllocateContent(PlaceholderName_1.default.Bottom, preAllocatedApplicationCustomizerBottomHeight, qosMonitor);
        }
    ...
    PlaceholderProvider.prototype.tryCreateContent = function (name, options) {
        sp_diagnostics_1._TraceLogger.logVerbose(PlaceholderProvider._logSource, "Creating placeholder content in placeholder \"".concat(name, "\""));
        var qosMonitor = new sp_diagnostics_1._QosMonitor('Placeholder.tryCreateContent');
        var content;
        if (name === PlaceholderName_1.default.Top && this._preAllocatedTopPlaceholderContent) {
            content = this._preAllocatedTopPlaceholderContent;
            this._preAllocatedTopPlaceholderContent = undefined;
        }
        else if (name === PlaceholderName_1.default.Bottom && this._preAllocatedBottomPlaceholderContent) {
    ...
    PlaceholderProvider.prototype._preAllocateContent = function (placeholderName, height, qosMonitor) {
        ...
        if (!this._placeholderManager.isEnabled || !this._placeholderManager.placeholders.has(placeholderName)) {
            var error = new Error("No placeholder found with the name '".concat(name, "'"));
        ...
        try {
            ...
            var shimmer = sp_component_base_1._ShimmerFactory.createShimmer(preAllocatedContent.domElement.clientWidth, height, PlaceholderName_1.default + this._componentTag, '', 0);
    ...
    PlaceholderProvider._logSource = sp_diagnostics_1._LogSource.create('PlaceholderProvider');
